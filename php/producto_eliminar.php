<?php
    ...
    $check_producto=$check_producto->query("SELECT * FROM product WHERE id='$product_id_del'");
    ...
    if($check_producto->rowCount()==1){
    	...
    	$eliminar_producto=$eliminar_producto->prepare("DELETE FROM product WHERE id=:id");

    	$eliminar_producto->execute([":id"=>$product_id_del]);

    	if($eliminar_producto->rowCount()==1){

    		if(is_file("./img/producto/".$datos['photo'])){
    			chmod("./img/producto/".$datos['photo'], 0777);
				unlink("./img/producto/".$datos['photo']);
    		}
